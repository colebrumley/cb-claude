...
To undo Claude's last turn of edits, use `/cb:rewind`. This runs `git reset --hard` to the most recent `[snapshot]` commit.

### Limitations

- **Single-undo only**: After rewinding, HEAD is on the snapshot commit. A second `/cb:rewind` finds the same commit — no-op. For deeper undo, use `git log --grep='\[snapshot\]'` to find older snapshots and `git reset --hard <hash>` manually.
...
- If you're not in a git repo — snapshots don't exist, /cb:rewind won't work
